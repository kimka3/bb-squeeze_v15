...
당일 승인된 수집·재시도·관측 기록 수정과 동일 캠페인 재개 검증은 [운영 수정 기록](deployments/2026-09-13-operational-repair.md)에 보존했다.

9월 22일·26일 재시작은 Ubuntu 보안 업데이트 후 `needrestart`가 수행한 것으로 확인했다. 보안 업데이트를 유지하면서 이번 봇의 자동 재시작만 고정 종료 시각까지 유예한 설정과 검증은 [재시작 원인 및 수정 기록](deployments/2026-09-26-update-restart-repair.md)에 보존했다. 유예 중 중요 보안 패치의 즉시 반영이 필요하면 점검 후 통제된 재시작을 해야 하며, 종료 시 미반영 라이브러리와 커널 재부팅 필요 여부를 확인한다.
...
| 변경 정책 | 전략·종목·위험비율·비용 모형·설정·패키지를 고정한다. 사용자가 승인한 수집·재시도·관측 기록의 운영 보수는 서비스를 일시 정지하고 원본 및 수정 이력을 보존한 뒤 같은 종료 시각으로 재개할 수 있다. 전략 변경은 별도 평가가 필요하다. |
...
봉 수집은 전체 종목에 같은 확정 봉 종료 시각과 기존 지표 입력 길이를 적용한다. 일시적인 봉 수집 실패는 보호 주문 점검 주기와 별개로 5·10·20초 이후 최대 30초 간격으로 재시도한다. 이 값은 신규 운영 설정이며 인수인계 문서에 없음이다. 기존 진입 유예 시간을 넘은 봉에서는 신규 진입을 계속 차단한다.

`state/runtime_health.json`은 시세 점검·봉·호가 관측·정산 서비스의 실패와 복구, 지연된 봉 및 재시작 관측 공백을 보존한다. 과거 저널에서 확인할 수 없는 정확한 복구 시각은 미상으로 남긴다. `report.json`의 `ledger_performance_complete`는 모의 장부의 기존 비용·체결 측정 범위만 나타내며, `observation_coverage_complete`는 관측 품질을 별도로 나타낸다. 관측 공백이 있으면 종합 `performance_complete`는 false다. 잔액이 변하지 않았거나 포지션이 없어도 데이터 공백이 없어지는 것은 아니다.

운영 보수에도 실행 지문 검사를 해제하지 않는다. `src/live/revision.py`는 일시 정지와 잠금 해제, 예상 캠페인 및 이전 지문·커밋, 새 커밋, 변경 경로와 고정 설정을 검사한다. 허용된 운영 소스만 변경할 수 있으며, `revisions/`에 이전·이후 캠페인 원본과 변경 사유·해시를 남긴다. 최초 시작·종료 시각, 설정과 장부는 유지한다. 배포 중단 구간은 관측 공백이며 기간을 연장하지 않는다. 수정 전후 결과가 서로 다른 운영 버전에서 측정됐다는 사실을 최종 평가에도 표시한다.
