...
현재 상태: 사용자의 상시 Linux/VPS가 아직 준비되지 않아 실제 캠페인은 시작하지 않았다. 이 문서는 새 서버에서 설치·검증·시작·감시·중지·결과 회수까지 수행하기 위한 절차다. 시작을 실행해야 시작 시각과 종료 시각이 생성된다.
...
서비스의 선행 점검도 새 캠페인 생성 전에 수행한다. `campaign.json`이 이미 있으면 복구 시작 때 불필요한 네트워크 점검을 강제하지 않아, 인터넷 장애 중에도 기존 종료 시각에 도달한 캠페인을 종료할 수 있다. 실행 시간은 서버 응답과 첫 점검에 따라 결정되므로 지금 종료 날짜를 사실로 제시할 수는 없다.
